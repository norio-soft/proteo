...
{
    public partial class ResourceManifest : Orchestrator.Base.BasePage
    {
        #region Properties
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            this.grdResourceManifestAddJobs.NeedDataSource += new GridNeedDataSourceEventHandler(grdResourceManifestAddJobs_NeedDataSource);
            this.grdResourceManifestAddJobs.ItemDataBound += new GridItemEventHandler(grdResourceManifestAddJobs_ItemDataBound);
        ...
        private string GetInstructionText(DataRow row)
        {
            ...
            if (InstructionTypeID == 1)
            {
                ...
                foreach (string order in Orders)
                {
                    ...
                    if (OrderDetails.Length > 1)
                    {
                        ...
                        actions += string.Format("Loading {0} Pallets for {1} at <b>{2} ({3})</b> using Vehicle:{4} Trailer:{5}", OrderDetails[5], OrderDetails[2], row["PostTown"], plannedArrivalDateTime.ToString("dd/MM HH:mm"), row["RegNo"].ToString(), row["TrailerRef"].ToString());
                    }
            ...
            if (InstructionTypeID == 2)
            {
                ...
                foreach (string order in Orders)
                {
                    ...
                    if (actions.Length > 0)
                        actions += "<br/>";
                    if (OrderDetails.Length > 1)
                        actions += string.Format("Dropping {0} pallets for {1} at <b>{2} ({3})</b> using Vehicle:{4} Trailer:{5}", OrderDetails[5], OrderDetails[2], row["PostTown"], plannedArrivalDateTime.ToString("dd/MM HH:mm"), row["RegNo"].ToString(), row["TrailerRef"].ToString());
                    OrderIDs = GetOrderIDs(row);
                }
            ...
            if (InstructionTypeID == 7)
            {
                ...
                actions += string.Format("Trunk to <b>{0} ({1})</b>  using Vehcile:{2} Trailer:{3}", row["PostTown"], plannedArrivalDateTime.ToString("dd/MM HH:mm"), row["RegNo"], row["TrailerRef"]);
                OrderIDs = GetOrderIDs(row);
        ...
        void grdResourceManifestAddJobs_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is Telerik.Web.UI.GridDataItem)
            {
                CheckBox chkDriverManifest = e.Item.FindControl("chkDriverManifest") as CheckBox;
