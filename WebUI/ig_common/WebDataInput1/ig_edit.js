...
function igedit_getById(id, elem)
{
	...
	if((o = igedit_all[id]) == null) for(var i in igedit_all) if((o = igedit_all[i]) != null)
		if(o.ID == id || o.uniqueId == id) break; else o = null;
...
function igedit_number(elem, id, prop0, prop1)
{
	...
	me.setValue = function(v)
	{
		...
		this.repaint();
		if(this.fcs == 2) this.select(1000);
	}
	me.spin = function(v)
	{
...
function igedit_date(elem, id, prop0, prop1)
{
	...
	me.focusTxt = function(foc, e, t)
	{
		...
		if(t == null)
		{
			...
			else if(!this.isNull) d = this.SelectedDate.Value;
		}
	...
	me.spin = function(v)
	{
		...
		if(i < 0 || this.limits(d) != null) return;
		this.text = this.toTxt(d, this.fcs == 2, this.promptChar, true);
		this.date = d;
		this.isNull = false;
		this.updatePost(d, "");
		this.repaint();
		if(this.fcs == 2) this.select(this.sel0);
	}
...
function igedit_mask(elem, id, prop0, prop1)
{
	...
	me.setValue = function(v){this.setTxt(v, (this.fcs < 0) ? 2 : this.mode);}
	me.getRenderedValue = function(v)
	{
		v = (v == null) ? "" : v.toString();
...
function igedit_new(elem, id, prop0)
{
	...
	if((e = ig_csom.getElementById(id)) == null) e = new Object();
	this.elemValue = e;
	//i=propCounter
	...
	if(ii != null && ii > 5)
	{
		e.style.height = ii;
		o = this.buttons;
		if(o != null && o[1] != null) if((o = o[1].elem) != null) for(j = 0; j < 4; j++)
		{
			o = o.parentNode;
			if(o.tagName == "TABLE" && o.offsetHeight < ii){o.style.height = ii; break;}
		}
	}
	//
	this.ID = id;
	...
	{
		var d, s0 = this.Element, s1 = this.elem, wd = this.wd, hd = this.hd;
		s0.style.display = v ? "" : "none";
		s0.style.visibility = v ? "visible" : "hidden";
		if(wd == null)
		{
			wd = s0.offsetWidth; hd = s0.offsetHeight; d = s1.offsetWidth;
			if(wd == null || wd < 5 || d == null || d < 5) wd = 0;
			else if((wd -= d) < 0) wd = 0;
			d = s1.offsetHeight;
			if(hd == null || hd < 5 || d == null || d < 5) hd = 0;
			else if((hd -= d) < 0) hd = 0;
			this.wd = wd; this.hd = hd;
		}
	...
	this.setText = function(v){this.elemValue.value = this.elemViewState.value = this.text = v; this.repaint();}
	...
	this.doEvt = function(e)
	{
		...
		//cut from mask
		if(type == 5 && this.fcs == 2) this.getSelectedText();
		if(v >= 0){this.doBut(e, type, v); return;}
		if(type != 1) if(this.fireEvent(type, e)) if(type < 4){ig_cancelEvent(e); return;}
