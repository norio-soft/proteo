...
Version 3.2.20042.26 (2004 Volume 2)
...
function igtree_initNode(node)
{
	...
	this.remove=function() {
		...
		var nodeElements;
		if(parentNode == null) {
			nodeElements = igtree_getChildNodeElements(tree.Id, null);
		}
		else {
			nodeElements = igtree_getChildNodeElements(tree.Id, parentNode.Element);
			if(nodeElements.length == 0) {
				if(!tree.ClassicTree && tree.Expandable) {
					if(parentNode.Element.childNodes[1].exp == true) {
						parentNode.Element.childNodes[2].innerHTML="";
						parentNode.Element.childNodes[1].removeNode();
						parentNode.Element.childNodes[1].removeNode();
		...
		igtree_setChildIds(tree.Id, nodeElements, index, parentId);
...
function igtree_insertChild(parentNode, index, text, className) {
	...
	if(insert) { // Perform insert
		if(parentNode == null) {
			nodeElements = igtree_getChildNodeElements(this.Id, null);
		}
		else {
			nodeElements = igtree_getChildNodeElements(this.Id, parentNode.Element);
		}
		...
		igtree_setChildIds(this.Id, nodeElements, index+1, parId);
	}
	else  
...
function igtree_setChildIds(treeName, nodeElements, index, parentId) {
	...
	for(i=index;i<len;i++) {
		...
		if(bSetChildren)
			childNodeElements = igtree_getChildNodeElements(treeName, nodeElement);
			
		//nodeElement.oldId = nodeElement.id;		
		nodeElement.id = id;
		if(nodeElement.oNode != null)
			nodeElement.oNode.Id = id;
		if(bSetChildren) {
			igtree_setChildIds(treeName, childNodeElements, 0, id);
		}
...
function igtree_getChildNodeElements(treename, parentElement) {
	elements=new Array();
	var nodeCount=0;
	var element=igtree_nextSiblingAll(treename, parentElement,null);
	while(element)	{
		elements[nodeCount++]=element;
		element=igtree_nextSiblingAll(treename, element, true);
	}
	return elements;
}
