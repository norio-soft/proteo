...
{
    public partial class scanDocumentNew : Orchestrator.Base.BasePage
    {
        //--------------------------------------------------------------------------
        ...
        private bool IsNewDynamicWebTwainLicensed
        {
            get
            {
                return File.Exists(Server.MapPath(NEW_WEBTWAIN_LIC_PATH));
            }
        }

        public string DynamicWebTwainLicense
        {
            get
            {
                if (IsNewDynamicWebTwainLicensed)
                {
                    return File.ReadAllText(Server.MapPath(NEW_WEBTWAIN_LIC_PATH));
                }
