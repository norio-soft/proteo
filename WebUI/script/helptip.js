...
function showHelpTipUrl(e, url) {
   ...
   timeOutID = setTimeout("_showTooltip()", 1000);
   //_showTooltip();
...
var helpTipHandler = {
	...
	positionToolTip:	function (e) {
		...
		// left
		if (_x > scroll.width - d.offsetWidth)
			d.style.left = scroll.width - d.offsetWidth + scroll.left + "px";
		else
			d.style.left = _x - 2 + scroll.left + "px";
		
		// top
		if (_y + d.offsetHeight + 18 < scroll.height)
			d.style.top = _y + 18 + scroll.top + "px";
		else if (_y - d.offsetHeight > 0)
			d.style.top = _y + scroll.top - d.offsetHeight + "px";
		else
