...
namespace Orchestrator.WebUI.Invoicing.SubContractorSB
{
	...
	public partial class AddUpdateExtraInvoice : Orchestrator.Base.BasePage
	{
		...
		private void LoadInvoiceExtra()
		{
			...
			lblSubContractor.Text = GetInvoiceSubContractor();
            ...
            //The SelfBillInvoiceNumber is usually used to store the Invoice number of Clients
            //who have sent a Self Bill, but in this case we are using it for Subbies who have
            //sent an invoice to us. 
            txtSubbyInvoiceNo.Text = m_invoiceExtra.SelfBillInvoiceNumber;
		...
		private void PopulateInvoiceExtra()
		{
            ...
            Entities.Organisation orgSubby = null;
            Facade.Organisation facOrg = new Orchestrator.Facade.Organisation();
            orgSubby = facOrg.GetForIdentityId(m_identityId);
            m_invoiceExtra.AccountCode = orgSubby.AccountCode;

            m_invoiceExtra.InvoiceDate = rdiInvoiceDate.SelectedDate.Value;
			m_invoiceExtra.InvoiceType = eInvoiceType.SubContractorExtra;
            ...
            //who have sent a Self Bill, but in this case we are using it for Subbies who have
            //sent an invoice to us. 
            if (!chkSelfBill.Checked)
                m_invoiceExtra.SelfBillInvoiceNumber = txtSubbyInvoiceNo.Text;
		}
		...
		private string GetInvoiceSubContractor()
		{
			if (m_isUpdate)
			{
                //GetClientForInvoiceExtraId works for SubContractors too
				Facade.IInvoiceExtra facInvoiceExtra = new Facade.Invoice();
				DataSet dsSubContractor = facInvoiceExtra.GetClientForInvoiceExtraId(Convert.ToInt32(ViewState["InvoiceId"]));
				
				m_identityId = Convert.ToInt32(dsSubContractor.Tables[0].Rows[0]["IdentityId"]);
				ViewState["IdentityId"] = m_identityId;

                return dsSubContractor.Tables[0].Rows[0]["OrganisationName"].ToString();
			}
			else
			{
...
//				reportParams.Add("InvoiceDate", lblDateCreated.Text);
			...
			reportParams.Add("OrganisationName", lblSubContractor.Text);

			// Invoice Date
			reportParams.Add("InvoiceDate", rdiInvoiceDate.SelectedDate.Value.ToShortDateString ());
        ...
        void btnChangeExtras_Click(object sender, EventArgs e)
        {
