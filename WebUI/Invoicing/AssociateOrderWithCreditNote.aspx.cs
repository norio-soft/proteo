...
{
    public partial class AssociateOrderWithCreditNote : Orchestrator.Base.BasePage
    {
        #region Constants

        private const string _creditNoteID_QS = "cnid";
        ...
        {
            if (!string.IsNullOrEmpty(Request.QueryString[_creditNoteID_QS]))
                int.TryParse(Request.QueryString[_creditNoteID_QS], out _creditNoteID);
        ...
        //TODO: Change to work with tblOrderCreditNotes table.
        void btnAssociate_Click(object sender, EventArgs e)
        {
            ...
            if (orderIDs.Count > 0)
            {
                Facade.IOrderGroup facOrderGroup = new Facade.Order();
                FacadeResult result =
                    facOrderGroup.AddOrdersToGroup(_creditNoteID, orderIDs, ((CustomPrincipal)Page.User).UserName);
                if (result.Success)
                {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                if (chk != null)
                {
                    chk.Attributes.Add("onclick",
                                       ...
                                       "," + ((DataRowView) e.Item.DataItem)["NoPallets"] + ");");

                    int orderGroupID;
                    int.TryParse(drv["OrderGroupID"].ToString(), out orderGroupID);
                    chk.Enabled = orderGroupID == 0;
                    if (orderGroupID == _creditNoteID)
                        chk.Checked = true;
                }
            }
        }
