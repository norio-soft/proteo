...
namespace Orchestrator.WebUI.Invoicing
{
    ...
    public partial class InvoiceExtraPreparation : Orchestrator.Base.BasePage
    {
        ...
        private DataSet PerformExtraSearch()
        {
            ...
            if (dteDateFrom.DateInput.Text != "" && dteDateTo.DateInput.Text != "")
            {
            ...
            if (dsExtras.Tables[0].Rows.Count == 0 || cboClient.SelectedValue == "")
            ...
            if (dsExtras.Tables[0].Rows.Count > 0)
            {
                if (Convert.ToInt32(dsExtras.Tables[0].Rows[0]["OnHold"]) == 1)
                {
                    ...
                    lblExtrasOnHold.Text = cboClient.Text + "'s account has been put on hold, please go to <A HREF=../Organisation/addupdateorganisation.aspx?IdentityId=" + Convert.ToInt32(cboClient.SelectedValue) + ">" + cboClient.Text + "'s details to change.</A>";
                }
                else
        ...
        private void BindData()
        {
            ...
            string sortBy = (SortCriteria + " " + (string.IsNullOrEmpty(SortDir) ? "" : SortDir == "Ascending" ? "Asc" : "Desc")).Trim();
        ...
        private void InitializeComponent()
        {
            ...
            this.gvExtras.Sorting += new GridViewSortEventHandler(gvExtras_Sorting);
        }

        void gvExtras_Sorting(object sender, GridViewSortEventArgs e)
        {

            if (!string.IsNullOrEmpty(SortDir) && SortCriteria == e.SortExpression)
            {
		...
		private void btnCreateInvoice_Click(object sender, EventArgs e)
		{
            if (Page.IsValid)
                if (this.SameCurrencyValidation())
                {
                    ...
                    Session["ExtraIds"] = arrExtraId;
                    Session["IdentityId"] = cboClient.SelectedValue;

                    Response.Redirect("AddUpdateExtraInvoice.aspx");
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkIncludeExtra != null)
                {
                    if (extraState != eExtraState.Accepted || Convert.ToBoolean(drv["BeingInvoiced"]))
                    {
                        ...
                        chkIncludeExtra.ToolTip = "This Extra is on a Pre-Invoice.";
                    }
                    else
                    {
                ...
                // Order Id column
                HtmlAnchor lnkOrderId = e.Row.FindControl("lnkOrderId") as HtmlAnchor;
                if (drv["OrderID"] != DBNull.Value)
                {
                    lnkOrderId.HRef = string.Format("javascript:openOrderProfile({0});", (int)drv["OrderID"]);
                    lnkOrderId.InnerText = drv["OrderID"].ToString();
                }

                // Job Id column
                HtmlAnchor lnkRunId = e.Row.FindControl("lnkRunId") as HtmlAnchor;
                if (drv["JobID"] != DBNull.Value)
                {
                    lnkRunId.HRef = string.Format("javascript:viewJobDetails({0});", (int)drv["JobID"]);
                    lnkRunId.InnerText = drv["JobID"].ToString();
                }
            }
        }
        ...
        {
            if (dteDateFrom.DateInput.Text != string.Empty && dteDateTo.DateInput.Text != string.Empty)
            {
