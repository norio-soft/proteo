...
function igdrp_getPosition(el){
	if(el!=null&&(el.style.position=="relative"||el.style.position=="absolute")){
		...
		if(el.style.position=="relative"){
			lx=(lx+el.offsetLeft)/2;
			ly=(ly+el.offsetTop)/2;
		}
		return {x:lx,y:ly}
	}
	for (var lx=0,ly=0;!igdrp_hasPosition(el);lx+=(el.offsetLeft-el.scrollLeft),ly+=(el.offsetTop-el.scrollTop),el=el.offsetParent);	
	return {x:lx,y:ly}
}
function igdrp_hasPosition(el){
	if(el==null)return true;
	return (el.style.position=="relative"||(el.offsetParent!=null&&el.offsetParent.style.position=="relative")||el.style.position=="absolute"||el.style.overflow=="auto"||el.style.overflow=="scroll"||el.scrollTop||el.scrollLeft)
}
...
if(typeof(igdrp_comboState) != "object")
...
function igdrp_combo(comboElement,comboProps) {
	...
	this.setEnabled		=	function(enable){this.Element.disabled=!enable;	if(this.editor)this.editor.setEnabled(enable);ig_ClientState.setPropertyValue(this.stateNode,"Enabled",enable);this.updatePostField();}
	...
	this.updatePostField=	function(value){if(this.postField!=null)
	...
	this.setDropDownVisible	=	function(bDrop,setFocus,innerCtrlElement)
	{
		var evt;
		...
		if(bDrop == true &&!this.isReadOnly())
		{
			...
			igdrp_setPanelPosition(this.Element,this.container,this.getDropDownAlignment(),this.transPanel, innerCtrlElement);
	...
	this.setFocusTop= function()
	{
		if(this.isEditable() == true) {
			...
			if((ig.IsIE || ig.IsNetscape6)&&this.getVisible())
				this.inputBox.focus();
		}
		else 
		{
			...
			if(ig.IsIE || ig.IsNetscape6) this.inputBox.focus();
		}
	}
	this.updateValue= function(newValue, suppressEvent) 
	{
		...
		var evt=igdrp_fireEvent(this, this.Events.ValueChanged[0], this.get_value());
	...
	this.onKeyDown=function(oEditor,value,oEvent){
		...
		switch(keyCode){
			...
			case 13:
				oCombo.setDropDownVisible(false);
				oCombo.updateValue(oCombo.getText(),false);
				break;
		}
	...
	ig.addEventListener(window,"unload",igdrp_onUnload,false);

}
function igdrp_onUnload(){
	ig_dispose(igdrp_comboState);
}
...
function igdrp_setPanelPosition(element,panel,panelAlignment,transLayer,innerCtrlElement){
	...
	//var w = parseInt(element.offsetWidth) - panel.offsetWidth;
	var w = innerCtrlElement && innerCtrlElement.offsetWidth?(parseInt(element.offsetWidth) - innerCtrlElement.offsetWidth) :(parseInt(element.offsetWidth) - panel.offsetWidth);
	...
	panel.style.left=	pxLeft	=pxLeft-(pxLeft+panel.offsetWidth>limits.maxX?((pxLeft+panel.offsetWidth)-limits.maxX):0);
