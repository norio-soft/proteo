	...
	if(se.tagName == "TH")
	{
		...
		if(igtbl_button(gn,evnt)==0)
		{
			...
			if(mode!=null && mode=="resize") 
			{
				...
				var c1w=co.offsetWidth+(evnt.clientX-(igtbl_getLeftPos(resCol)+co.offsetWidth-(resCol.clientLeft?resCol.clientLeft:0)))+4;
				igtbl_resizeColumn(gn,resCol.id,c1w);
				if(typeof(gs.Element.parentNode.scrollLeft)!="undefined")
				{
					var actWidth=gs.Element.offsetWidth-gs.Element.parentNode.scrollLeft;
					if(actWidth<evnt.clientX+4)
						gs.Element.parentNode.scrollLeft+=evnt.clientX-actWidth+4;
				}
...
function igtbl_cellClickUp(evnt,gn)
{
	...
	var bandNo=se.parentNode.parentNode.parentNode.getAttribute("bandNo");
	var fac=igtbl_getBandFAC(gn,se);
	if(se.cellIndex>fac-1 && igtbl_getCellClickAction(gn,bandNo)==1)
	{
		if(igtbl_getAllowUpdate(gn,bandNo,igtbl_getColumnNo(gn,se))==3)
			igtbl_getRowById(se.parentNode.id).editRow(true);
		else
...
function igtbl_selectCell(gn,cellID,selFlag,fireEvent)
{
	...
	else
	{
		if(gs.SelectedCells[cellID])
		{
			...
			var scr=gs.SelectedCellsRows[cell.Element.parentNode.id];
			if(scr && scr[cellID])
				delete scr[cellID];
		}
...
function igtbl_endCustomEdit()
{
	...
	if(se!=null)
	{
		...
		if(se.getAttribute("editorControl"))
		{
			...
			{
				oEditor.setValue(cellObj.get_value(),false);
				oEvent.cancel=true;
				return;
			}
			if(typeof(oEditor.get_value())!="undefined")
				cellObj.setValue(oEditor.get_value());
