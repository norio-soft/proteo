...
function igtbl_addNew(gn,bandNo)
{
	...
	if(lastRow)
	{
		...
		var rn=lrObj.OwnerCollection.getLastRowId().split("_");
	...
	for(var i=0;i<g.Bands[bandNo].Columns.length+fac;i++)
	{
		...
		else
		{
			...
			if(column.CssClass)
				...
				cell.className=g.Bands[bandNo].getAltClass();
			else
				cell.className=g.Bands[bandNo].getItemClass();
			if(column.Style)
	...
	for(var i=0;i<rowObj.cells.length;i++)
	{
		...
		cellObj.setValue(cellObj.Column.DefaultValue);
	}
