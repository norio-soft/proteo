...
function igtbl_grid(_gridElement,gridProps,gridEvents)
{
	...
	this.ClientID=gridProps[51];
	this.DefaultCentury=gridProps[52];
	...
	{
		return igtbl_fireEvent(this.Id,eventName,args);
	}
	...
	this.update=function()
	{
		...
		if(!p) return;
		while(this.ViewState.childNodes.length>0)
			this.ViewState.removeChild(this.ViewState.firstChild);
		var node,cs=ig_ClientState;
		if(igtbl_getLength(this.DeletedRows)>0)
		{
			node=cs.addNode(this.ViewState,"DeletedRows");
		...
		{
			node=cs.addNode(this.ViewState,"AddedRows");
			for(var row in this.AddedRows)
			{
				var rowObj=igtbl_getElementById(row);
				if(rowObj && this.AddedRows[row])
				{
					var rn=cs.addNode(node,row);
					if(rowObj.getAttribute("level"))
						cs.setPropertyValue(rn,"level",rowObj.getAttribute("level"));
				}
			}
		}
		if(igtbl_getLength(this.SelectedRows)>0)
		{
			node=cs.addNode(this.ViewState,"SelectedRows");
		...
		{
			node=cs.addNode(this.ViewState,"ExpandedRows");
		...
		{
			node=cs.addNode(this.ViewState,"CollapsedRows");
		...
		{
			node=cs.addNode(this.ViewState,"ResizedRows");
			...
			for(var cell in this.ChangedCells)
			{
				...
				if(cellObj)
				{
					...
					if(val!=null && val!="" && typeof(val)=="string")
						val=val.replace(/\+/g,"\\x2B");
					if(val!=null && !(typeof(val)=="string" && val==""))
						cs.setPropertyValue(cn,"value",val.toString());
				}
			}
		}
		if(this.oActiveCell)
		{
			var ac=cs.addNode(this.ViewState,"ActiveCell");
			cs.setPropertyValue(ac,"value",this.oActiveCell.Element.id);
			if(this.oActiveCell.Element.getAttribute("level"))
				cs.setPropertyValue(ac,"level",this.oActiveCell.Element.getAttribute("level"));
		}
		else if(this.oActiveRow)
		{
			var ar=cs.addNode(this.ViewState,"ActiveRow");
			cs.setPropertyValue(ar,"value",this.oActiveRow.Element.id);
			if(this.oActiveRow.Element.getAttribute("level"))
				cs.setPropertyValue(ar,"level",this.oActiveRow.Element.getAttribute("level"));
		}
		node=null;
		for(var i=0;i<this.Bands.length;i++)
		{
			...
			if(!node && band.SortedColumns && band.SortedColumns.length>0)
				node=cs.addNode(this.ViewState,"SortedColumns");
			for(var j=0;j<band.SortedColumns.length;j++)
			{
				var col=band.SortedColumns[j];
				var cn=cs.addNode(node,col);
				cs.setPropertyValue(cn,"sortIndicator",band.Columns[igtbl_colNoFromColId(col)].SortIndicator.toString());
				cs.setPropertyValue(cn,"lastSorted",(col==this.lastSortedColumn).toString());
			}
		}
		cs.setPropertyValue(cs.addNode(this.ViewState,"ScrollLeft"),"value",this.Element.parentNode.scrollLeft.toString());
		cs.setPropertyValue(cs.addNode(this.ViewState,"ScrollTop"),"value",this.Element.parentNode.scrollTop.toString());
		if(this.moveBackPostField)
			this.ViewState.appendChild(cs.removeNode(this.ViewState,this.moveBackPostField));
		p.value=cs.getText(this.ViewState.parentNode);
	}
...
function igtbl_band(grid, bandArray, count) 
{
	...
	this.DataKeyField=bandArray[43];
	...
	if(grid.AddNewBoxVisible)
	{
		...
		if(grid.AddNewBoxView==0)
			this.addNewElem = addNew.childNodes[0].rows[0].cells[1].childNodes[0].rows[count].cells[count];
		else
			this.addNewElem = addNew.childNodes[0].rows[0].cells[1].childNodes[0].rows[0].cells[count*2];
	}
...
function igtbl_column(band, colArray, index)
{
	...
	if(typeof(igcmbo_getComboById)!="undefined" && igcmbo_getComboById(this.EditorControlID))
...
function igtbl_initRowsCollection(parentRow,grid,bandNo,node)
{
	this.Type="rows";
	...
	this.getColumn=igtbl_clctnGetColumn;
	...
	this.setLastRowId=function(lrId)
	{
...
function igtbl_clctnGetRow(rowNo)
{
	...
	if(!this.rows[rowNo])
	{
		...
		if(!row)
			return null;
		this.rows[rowNo]=new igtbl_initRowInfo(this.Grid.Id,row,this,rowNo);
	}
...
function igtbl_clctnGetColumn(colNo)
{
	var thead=this.Element.previousSibling;
	if(!thead || thead.tagName!="THEAD")
		return;
	return thead.firstChild.cells[colNo+this.Band.firstActiveCell];
}
...
function igtbl_getCell(index)
{
	if(index<0 || !this.cells || index>this.cells.length)
		return null;
	if(!this.cells[index] && (this.Element.cells[this.Band.firstActiveCell+parseInt(index)]))
...
{
	return (this.Expandable && igtbl_getGridById(this.gridId).ExpandedRows[this.getFirstRow().id]);
}
...
{
	if(igtbl_getGridById(this.gridId).SelectedRows[this.getFirstRow().id])
		return true;
	return false;
}
...
function igtbl_initCellInfo(row,index)
{
	...
	this.renderActive=function(render)
	{
		...
		if(!(ig_csom.IsNetscape6 || ig_csom.IsNetscape))
			igtbl_changeBorder(g,this.Element.style,this,"",render);
...
function igtbl_changeBorder(g,elem,obj,attr,render)
{
	...
	else
	{
		...
		elem[attrStyle]=g.Activation.getValue(attr,"Style");
		if(typeof(obj["old"+attrColor])=="undefined")
			obj["old"+attrColor]=elem[attrColor];
		elem[attrColor]=g.Activation.getValue(attr,"Color");
		if(typeof(obj["old"+attrWidth])=="undefined")
			obj["old"+attrWidth]=elem[attrWidth];
		elem[attrWidth]=g.Activation.getValue(attr,"Width");
	}
}
...
function igtbl_setCellValue(value)
{
	...
	if(!gs.insideBeforeUpdate)
	{
		...
		res=igtbl_fireEvent(gn,gs.Events.BeforeCellUpdate,"(\""+gn+"\",\""+this.Element.id+"\",\""+ev+"\")");
	...
	if(typeof(value)!="undefined" && value!=null)
	{
		if(oldValue && oldValue.getMonth && typeof(value)=="string")
		{
			var year="";
	...
	else 
	{
		...
		else if(this.Column.Type==3 && this.Element.childNodes.length>0)
		{
			...
			if(chBox)
			{
				if(!value || value.toString().toLowerCase()=="false")
					...
					chBox.checked=true;
				this.Element.setAttribute("chkBoxState",chBox.checked.toString());
			}
		...
		if(setInner)
		{
			...
			if(this.Element.childNodes.length>0 && this.Element.childNodes[0].tagName=="NOBR")
			{
				...
				{
					igtbl_setInnerText(this.Element.childNodes[0].childNodes[0],vs);
					this.Element.childNodes[0].childNodes[0].href=(v.indexOf('@')>=0?"mailto:":"")+v;
				}
				else
					igtbl_setInnerText(this.Element.childNodes[0],vs);
			}
			else if(this.Element.childNodes.length>0 && this.Element.childNodes[0].tagName=="A")
			{
				...
				igtbl_setInnerText(this.Element,vs);
		}
	}
