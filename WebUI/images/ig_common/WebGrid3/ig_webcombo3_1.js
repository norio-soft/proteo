...
function igcmbo_arrowKeyNavigation(oCombo, oGrid, oRow, column){
	...
	if(oRow != null){
		...
		if(oCombo.DataValueField) oCombo.dataValue=oRow.getCellFromKey(oCombo.DataValueField).get_value();
			...
			if(charFromCode&&bCountableKey){				
				...
				if(cell!=null){
					...
					if (!oCombo.Editable){
						...
						if (oActRow) oEditor.value= oCombo.DataTextField!=null && oCombo.DataTextField!="" ? oActRow.getCellFromKey(oCombo.DataTextField).get_value() : oActRow.getCell(0).get_value(); 
...
function igcmbo_onblur(evnt,comboId) {
	...
	if (!oCombo.Editable){		
		...
		if (oActRow)
			{			
				var oCellValue = oCombo.DataTextField!=null && oCombo.DataTextField!="" ? oActRow.getCellFromKey(oCombo.DataTextField).get_value():oActRow.getCell(0).get_value(); 
...
function igcmbo_setSelectedRow(row) {
	...
	if(this.DataValueField.length > 0) 
	{
		...
		this.setDataValue(cell.get_value(), false);
		if(this.Element.style.display!="none")
			this.setFocusTop();
	}
}
...
function igcmbo_setDisplayValue(newValue, bFireEvent)
{
	...
	if(cell != null) {
		if(this.DataValueField)
			this.dataValue=cell.Row.getCellFromKey(this.DataValueField).get_value();
...
function igcmbo_setDataValue(newValue, bFireEvent)
{
	...
	if(cell != null)
	{
		if(this.DataTextField)
			this.updateValue(cell.Row.getCellFromKey(this.DataTextField).get_value(),bFireEvent);
		igtbl_clearSelectionAll(this.grid.Id);
		this.grid.setActiveRow(cell.Row);
		cell.Row.setSelected(true);
		this.selectedIndex = cell.Row.getIndex();
